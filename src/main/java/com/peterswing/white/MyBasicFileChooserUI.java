 ...
 * @author Jeff Dinkins
 */
public class MyBasicFileChooserUI extends javax.swing.plaf.FileChooserUI {
	...
	private FilterBasicDirectoryModel model = null;
	...
	public MyBasicFileChooserUI(JFileChooser b) {
	}
	...
	protected void createModel() {
		...
		model = new FilterBasicDirectoryModel(getFileChooser());
	}

	public FilterBasicDirectoryModel getModel() {
		return model;
	}
		...
		public void valueChanged(ListSelectionEvent evt) {
			if (!evt.getValueIsAdjusting()) {
				...
				if (chooser.isMultiSelectionEnabled()) {
					...
					if (objects != null) {
						if (objects.length == 1 && ((File) objects[0]).isDirectory() && chooser.isTraversable(((File) objects[0]))
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (filename != null && !filename.equals("")) {
				...
				if (chooser.isMultiSelectionEnabled() && filename.startsWith("\"")) {
					...
					boolean isCtrl = (e != null && (e.getModifiers() & Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()) != 0);
	...
	private static boolean isGlobPattern(String filename) {
		return ((File.separatorChar == '\\' && (filename.indexOf('*') >= 0 || filename.indexOf('?') >= 0)) || (File.separatorChar == '/' && (filename.indexOf('*') >= 0
	...
	private void changeDirectory(File dir) {
		...
		if (fc.getFileSelectionMode() == JFileChooser.FILES_AND_DIRECTORIES && fc.getFileSystemView().isFileSystem(dir)) {
